...
import { 
    ...
    Paper, Checkbox, Typography, Chip, Box, IconButton
} from '@mui/material';
...
const StatusChip: React.FC<{ status: string; type: 'approval' | 'publish' }> = ({ status, type }) => {
  ...
  if (type === 'approval') {
    switch (status.toLowerCase()) {
      case 'approved': bgColor = '#A5D6A7'; label = 'Approved'; break; // Green 200
      case 'rejected': bgColor = '#EF9A9A'; label = 'Rejected'; break; // Red 200
      case 'pending': bgColor = '#B2EBF2'; label = 'Under Review'; break; // Cyan 100
    }
  } else {
  ...
  // Adjust text color/font based on Figma's light aesthetic for approval, bold for publish
...
const SpecialistTable: React.FC = () => {
  ...
  // Pagination Controls (Custom Visual < Previous 1 2 3 Next >)
  const renderPagination = () => (
      <div className="flex items-center justify-center p-6 space-x-2">
        ...
        {[...Array(totalPages)].map((_, i) => {
             const pageNum = i + 1;
             // Logic to show generic page numbers (simplified for assessment scope)
             return (
                <button
                    key={pageNum}
      ...
      <TableContainer>
        <Table className='min-w-[900px]'>
          <TableHead>
            <TableRow>
              {columns.map((column) => (
                <TableCell key={column.id} width={column.width} sx={{ fontWeight: 600, color: '#9CA3AF', fontSize: '11px', textTransform: 'uppercase' }}>
          ...
          <TableBody>
            {loading ? (
                <TableRow><TableCell colSpan={8} align="center" className="py-10">Loading...</TableCell></TableRow>
            ) : list.map((row) => {
              ...
              return (
                <TableRow
                  ...
                  sx={{ '&.Mui-selected': { bgcolor: '#F0F7FF' }, borderBottom: '1px solid #F3F4F6' }}
                >
                  <TableCell padding="checkbox">
                    <Checkbox size='small' checked={isItemSelected} onChange={() => setSelected(prev => isItemSelected ? prev.filter(i => i!== row.id) : [...prev, row.id])} />
                  </TableCell>
                  <TableCell><Typography className="font-semibold text-sm">{row.title}</Typography></TableCell>
                  <TableCell>RM {row.final_price ? row.final_price.toLocaleString() : '0.00'}</TableCell>
                  ...
                  <TableCell><StatusChip status={row.is_draft ? 'Not Published' : 'Published'} type='publish' /></TableCell>
                  <TableCell>
                    <SpecialistActions specialistId={row.id} onDelete={handleDeleteSpecialist} />
                  </TableCell>
                </TableRow>
              );
            })}
